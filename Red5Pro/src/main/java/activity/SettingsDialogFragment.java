...
import fragment.SubscribeListFragment;
...
public class SettingsDialogFragment extends Fragment {
    ...
    private SubscribeListFragment streamList;
...
//            CheckBox cba = (CheckBox)v.findViewById(R.id.settings_adaptive_bitrate);
            ...
            switch (checkedID) {
                ...
                case R.id.settings_quality_other:
                    bitRate = PublishActivity.config.bitrate;
                    resolutionWidth = Integer.valueOf( PublishActivity.selected_item.split("x")[0] );
                    resolutionHeight = Integer.valueOf( PublishActivity.selected_item.split("x")[1] );
                    selectedQuality = 3;
                    break;
            }
            ...
            PublishActivity.preferedResolution = selectedQuality;
            PublishActivity.selected_item = resolutionWidth + "x" + resolutionHeight;
            PublishActivity.config.bitrate = bitRate;

            Log.d("SettingsDialogFragment", "PublishActivity.preferedResolution " + selectedQuality);
            Log.d("SettingsDialogFragment", "PublishActivity.selected_item " + resolutionWidth + "x" + resolutionHeight);
            Log.d("SettingsDialogFragment", "PublishActivity.config.bitrate " + bitRate);
...
//            editor.putBoolean(getPreferenceValue(R.string.preference_adaptive_bitrate), cba.isChecked());
    ...
    private void showUserSettings(View v) {
        ...
        PublishActivity.config.host = host;
...
//        EditText app = getField(v, R.id.settings_appname);
            ...
            case PUBLISH:
                ...
                switch (quality) {
                    case 0:
                        ...
                        PublishActivity.preferedResolution = 0;
                        PublishActivity.selected_item = "426x240";
                        PublishActivity.config.bitrate = 400;
                        break;
                    case 1:
                    default:
                        ((RadioButton)group.findViewById(R.id.settings_quality_medium)).setChecked(true);
                        PublishActivity.preferedResolution = 1;
                        PublishActivity.selected_item = "854x480";
                        PublishActivity.config.bitrate = 1000;
                        break;
                    case 2:
                        ((RadioButton)group.findViewById(R.id.settings_quality_high)).setChecked(true);
                        PublishActivity.preferedResolution = 2;
                        PublishActivity.selected_item = "1920x1080";
                        PublishActivity.config.bitrate = 4500;
                        break;
                    case 3:
                        ((RadioButton)group.findViewById(R.id.settings_quality_other)).setChecked(true);
                        PublishActivity.preferedResolution = 3;
                        PublishActivity.selected_item = preferences.getInt(getResources().getString(R.string.preference_resolutionWidth), getResources().getInteger(R.integer.preference_default_resolutionWidth)) +
                        ...
                        PublishActivity.config.bitrate = preferences.getInt(getResources().getString(R.string.preference_bitrate), getResources().getInteger(R.integer.preference_default_bitrate));
                        break;
                }
                break;
            case SUBSCRIBE:
                ...
                streamList = (SubscribeListFragment) getFragmentManager().findFragmentById(R.id.streamList);
                streamList.mCallbacks = new SubscribeListFragment.Callbacks() {
                    @Override
    ...
    private AdapterView.OnItemSelectedListener getItemSelectedHandlerForResolution(){
         return new AdapterView.OnItemSelectedListener() {
             ...
             public void onItemSelected(AdapterView<?> adapterView, View view, int i, long l) {
                 ...
                 PublishActivity.selected_item = String.valueOf(adapterView.getSelectedItem());
                 PublishActivity.preferedResolution = adapterView.getSelectedItemPosition();
             }
    ...
    public void switchToAdvanced(View v){
        ...
        if(state == AppState.PUBLISH) {
            ...
            //bitrate
            getField(advancedSubView, R.id.rateText).setText("" + PublishActivity.config.bitrate);
            //resolution
            getField(advancedSubView, R.id.resolutionText).setText(PublishActivity.selected_item);
            //audio check
        ...
        final Fragment thisFragment = this;
        // PublishActivity/SubscribeActivity
...
//                saveSettings(settingsSubView);
    ...
    public boolean saveAdvancedSettings(){
        ...
        if( state == AppState.PUBLISH ) {
            ...
            if(baseBitrate != newBitrate || !PublishActivity.selected_item.equalsIgnoreCase(newResolution) ) {
                ...
                PublishActivity.selected_item = newResolution;
            }
        }
